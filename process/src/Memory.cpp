...
std::vector<int> Memory::GetProcessId(const char* processName) {
    std::vector<int> pids;
    SetLastError(0);
    PROCESSENTRY32 pe32;
...
#if 0
    if(!EnumProcessModules(processHandle, hModules, 0, &cModules)){
        // error handle
        std::cout << "EnumProcessModules failed. Error: " << GetLastError() << std::endl;
        exit(-1);
    }
#endif
...
char* Memory::ReadText(HANDLE processHandle, long long address) {
    if (address == -1)
        return "-1";
    char buffer = !0;
...
bool Memory::WriteInt(HANDLE processHandle, long long address, int value) {
    if (address == -1)
        return false;
    SIZE_T NumberOfBytesToWrite = sizeof(value);
    SIZE_T NumberOfBytesActuallyWritten;
    BOOL success = WriteProcessMemory(processHandle, (LPVOID)address, &value, NumberOfBytesToWrite, &NumberOfBytesActuallyWritten);
    if (!success || NumberOfBytesActuallyWritten != NumberOfBytesToWrite)
        return false;
    return true;
}

bool Memory::WriteText(HANDLE processHandle, long long address, const char* value) {
    if (address == -1)
        return false;
    SIZE_T NumberOfBytesToWrite = strlen(value) + 1;
    SIZE_T NumberOfBytesActuallyWritten;
    BOOL success = WriteProcessMemory(processHandle, (LPVOID)address, value, NumberOfBytesToWrite, &NumberOfBytesActuallyWritten);
    if (!success || NumberOfBytesActuallyWritten != NumberOfBytesToWrite)
        return false;
